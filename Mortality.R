...
mort$mort2022 <- (mort$mort2022 / pop$pop2021) * 100

mort$region <- factor(mort$region, levels = c("Kedougou", "Tambacounda", "Kolda", "Senegal"))
...
coef <- max(inc_long$incidence) /
  max(mort_long$mortality)

mort_long$mortality_scaled <-
  mort_long$mortality * coef
